...
class Database:
    ...
    def __init__(self, db_file_name: Optional[str] = None):
        ...
        self.db_file_name = db_file_name or get_config_value(
            "bot_settings.db_file_name",
            "data/db/jfa_bot.db",  # Ensure default matches new structure
        )
        ...
        db_dir = os.path.dirname(self.db_file_name)
        if db_dir and not os.path.exists(db_dir):
            try:
                ...
                # Depending on desired behavior, could raise error or try to fallback
                # For now, allow _init_db to attempt creation which might fail visibly

        self._init_db()
        self.logger.info(f"Database initialized using file: {self.db_file_name}")
    ...
    def _init_db(self) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.commit()
                self.logger.info(
                    f"Database initialized successfully: {self.db_file_name}"
                )
        except Exception as e:
            self.logger.critical(
                f"Failed to initialize database {self.db_file_name}: {str(e)}"
            )
            raise
    ...
    def _get_connection(self) -> Generator[sqlite3.Connection, None, None]:
        ...
        try:
            ...
            self.logger.debug(f"Database connection opened: {self.db_file_name}")
            yield conn
        except sqlite3.Error as e:
            self.logger.error(f"Database error ({self.db_file_name}): {str(e)}")
            raise
        finally:
            if conn:
                conn.close()
                self.logger.debug(f"Database connection closed: {self.db_file_name}")
        ...
        try:
            with self._get_connection() as conn:
                ...
                if row:
                    status = row["status"]
                    self.logger.debug(
                        f"Found invite status '{status}' for user_id: {user_id}"
                    )
                    return status
                self.logger.debug(f"No invite record found for user_id: {user_id}")
                return None
    ...
    def find_invites_by_username_pattern(self, pattern: str) -> List[sqlite3.Row]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                results = cursor.fetchall()
                self.logger.debug(
                    f"Found {len(results)} invite records matching pattern: {pattern}"
                )
                return results
        except Exception as e:
            self.logger.error(
