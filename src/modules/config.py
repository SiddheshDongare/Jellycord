...
DEFAULT_CONFIG_FILE_PATH = "config/config.yaml"  # Updated path
DEFAULT_TEMPLATES_FILE_PATH = (
    "config/message_templates.json"  # Added for messaging module
)

DEFAULT_CONFIG_STRUCTURE = {
    "bot_settings": {
        "bot_name": "JFA-GO Invite Bot",
        "log_file_name": "logs/jfa_bot.log",
        "db_file_name": "data/db/jfa_bot.db",
        "debug_mode": False,
...
EXPECTED_CONFIG: Dict[str, Tuple[type, bool, Any]] = {
    ...
    "bot_settings.log_file_name": (str, False, "logs/jfa_bot.log"),
    "bot_settings.db_file_name": (str, False, "data/db/jfa_bot.db"),
    ...
    "message_settings.templates_file": (
        str,
        False,
        DEFAULT_TEMPLATES_FILE_PATH,  # Use the defined constant
    ),
...
def load_app_config(config_file_path: Optional[str] = None) -> dict:
    """
    ...
    actual_config_file_path = (
        config_file_path or DEFAULT_CONFIG_FILE_PATH
    )  # Use updated default
    logger.info(f"Attempting to load configuration from: {actual_config_file_path}")

    # Load YAML config first
    yaml_config = _load_yaml_config(actual_config_file_path)
