...
- The `/remove_invite [user_identifier]` command implements a multi-step process for comprehensive user and invite removal:
    1.  **User Identification (`admin_commands.py`):**
        - The input `user_identifier` is parsed to find a Discord user (by ID or mention) or a Jellyfin username.
        - If a Discord user is found, their ID is used to check the `jfa_user_cache` (via `db.get_jfa_user_from_cache_by_discord_id`) for a linked Jellyfin username.
        - If the input is treated as a Jellyfin username, it's checked directly against the `jfa_user_cache` (via `db.get_jfa_user_from_cache_by_jellyfin_username`).
    2.  **JFA-GO User Deletion (`jfa_client.delete_jfa_user_by_username`):
        - If a Jellyfin username is determined, this method is called.
        ...
        - Handles API errors and returns success/failure status.
    3.  **JFA-GO Invite Code Deletion (`admin_commands.py` using `jfa_client.delete_jfa_invite`):
    ...
    4.  **Local Database Status Update (`admin_commands.py` using `db.update_user_invite_status`):
    ...
    5.  **Role Reversion (`admin_commands.py` - not detailed here but part of the flow):**
        - Attempts to revert Discord roles assigned by the bot.
    6.  **Logging and Confirmation (`admin_commands.py`):
        - An `AdminAction` is recorded in the database.
        - A detailed confirmation embed summarizing all actions taken (and any errors) is sent to the invoking administrator.
